# Image WebP Migration & Cleanup Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Convert all in-use PNG, JPG, and JPEG images to WebP format, replace all code references with the WebP paths, and clean up the original in-use image files while preserving logos and the main hero background image.

**Architecture:** We will create a Python script `scratch/webp_migration.py` that crawls the website's HTML/CSS files, identifies used images, converts them to WebP using the Pillow library, modifies the HTML/CSS source code references, and safely deletes the original files.

**Tech Stack:** Python 3, PIL (Pillow), Git

## Global Constraints
* Do NOT convert or delete `logo.png` and `Just_logo.png`.
* Do NOT delete the original hero background `assets/img/New_Images/Implant_Section.jpeg`, but convert it and update its HTML references to WebP.
* Do NOT touch, convert, or delete any image files that are not referenced in the website's HTML or CSS code.

---

### Task 1: Create WebP Migration Python Script

**Files:**
- Create: `scratch/webp_migration.py`

**Interfaces:**
- Produces: `scratch/webp_migration.py` script that can be executed to process the migration.

- [ ] **Step 1: Write the migration script code**

Write the complete script to `scratch/webp_migration.py`:
```python
import os
import re
...
```bash
git add scratch/webp_migration.py
git commit -m "chore: create webp migration script"
```

---

### Task 2: Run Migration Script and Verify Results

**Files:**
- Modify: All HTML files in root directory, CSS files in `css/`
- Delete: Original PNG, JPG, JPEG files in `assets/img/` (excluding logos and `Implant_Section.jpeg`)

**Interfaces:**
- Consumes: `scratch/webp_migration.py`

- [ ] **Step 1: Execute Python migration script**

Run: `python scratch/webp_migration.py`
Expected: Outputs list of converted files, updated references, and deleted files.

- [ ] **Step 2: Verify git status and check specific files**

Verify that:
1. `logo.png` and `Just_logo.png` are untouched.
2. `assets/img/New_Images/Implant_Section.jpeg` is NOT deleted.
3. Original files like `Teeth-alignment.jpg` are deleted.
4. `.html` and `.css` files are updated.

Run: `git status`

- [ ] **Step 3: Commit migration results**

```bash
git add .
git commit -m "style: migrate referenced images to WebP and clean up original files"
```

- [ ] **Step 4: Push changes to GitHub**

Run: `git push`
Expected: Push succeeds and updates remote repository.
