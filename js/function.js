(function ($) {
	...
	$mobileMenu.find('li.submenu').each(function() {
		var $a = $(this).children('a');
		if ($a.attr('href') === 'treatments.html' || $a.text().trim().toLowerCase().includes('treatments')) {
			$(this).removeClass('submenu');
			$(this).children('ul').remove();
		}
	});
	$mobileMenu.attr('id', 'menu-mobile').css('display', 'none').appendTo('body');
	...
	/* Close mobile menu on tap/click outside */
	$(document).on('click touchstart', function(e) {
		var $nav = $('.slicknav_nav');
		var $btn = $('.navbar-toggle .slicknav_btn');
		if ($nav.is(':visible') && !$(e.target).closest('.navbar-toggle, .responsive-menu').length) {
			$btn.trigger('click');
		}
	});
	...
	$('.exp-years-Vivekanand').text(currentYear - 2001);
